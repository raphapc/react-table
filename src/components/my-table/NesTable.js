...
import React, { cloneElement, Fragment, useEffect, useImperativeHandle, useState } from 'react';
...
const NesTable = React.forwardRef(
  (
    {
      dataSource,
      width,
      ...
      return (
        <div className={className} role="row" style={style}>
          {React.Children.map(columns, (column, index) => {
            ...
            const sorted = sortFilter.find(
              (filter) => filter.key === column.props.id && (filter.isSorted || filter.filterValue)
            );
              ...
              <Fragment>
                <CustomHeader
                  className={classNames(
                    { 'drag-header': sorteableHeader, 'simple-header': !sorteableHeader },
                    headerId,
                    headerClass
                  )}
                  index={index}
        ...
        filterOptions.push(
          ...
          >
            {getMenuItem({
              ...
              inputRef2: filterOptionalInputRef,
              columnType,
              idColumn,
              operation: numberFilters.between,
            })}
          </SubMenu>
        );
      } else if (columnType?.type === 'date') {
        ...
        filterOptions.push(
          ...
          >
            {getMenuItemDate({
              ...
              columnType,
              idColumn,
              operation: dateFilters.interval,
            })}
          </SubMenu>
        );
      } else {
    ...
    const getMenuItemDate = ({ inputRef, inputRef2, columnType, idColumn, operation }) => {
      return (
        <Menu.Item>
          ...
          >
            {!inputRef2 && (
              <DatePicker className="date-picker" format={'DD-MM-YYYY'} onChange={(value) => (filterDates = [value])} />
            )}
            {inputRef2 && (
              <DatePicker.RangePicker
                className="range-picker"
                format={'DD-MM-YYYY'}
                onChange={(value) => (filterDates = value)}
              />
            )}
            <Button
              type="primary"
    ...
    const selectCheckbox = (rowSelectedIndex) => {
      ...
      setSortedList((prevSortedList) => {
        ...
        const list = newSortedList.map((row, index) => {
    ...
    const renderRow = (props, expandedRows) => {
          ...
          <Fragment key={key}>
            <div className="" style={style}>
              ...
              <div
                ...
                style={{ height: secondGlanceHeight, paddingLeft: paddingLeft, background: bgExpanded }}
              >
      ...
      // if (clickedRowClassName && clickedRow && equals(clickedRow.id, rowData.id)) {
      //   props.className = classNames(props.className, clickedRowClassName);
      // }
      return (
        <Fragment key={key}>
            ...
            sortedList && (
              <DragColumn {...dragProps}>
                <Table
                  ...
                  className={`${tableClassName} nes-table`}
                  rowClassName={rowClassName}
                  rowRenderer={(renderParams) => renderRow(renderParams, expandedRows)}
                  onRowClick={({ event, index, rowData }) => onRowClicked(event, index, rowData)}
                >
                  {lineSelectable && (
                    <Column
                      id={'checkbox'}
